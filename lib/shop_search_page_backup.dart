    ...
    try {
      ...
      setState(() {
        ...
        if (isSimulatorLocation) {
          _statusMessage = '⚠️ SIMULATOR DETECTED\nLocation: ${position.latitude.toStringAsFixed(4)}, ${position.longitude.toStringAsFixed(4)} (San Francisco)\n\nThis will show San Francisco stores. For your actual area, use "Set Location Manually".';
        } else {
          _statusMessage = 'Location found: ${position.latitude.toStringAsFixed(4)}, ${position.longitude.toStringAsFixed(4)}\nSearching for stores...';
        }
      });

      print(isSimulatorLocation ? '⚠️ STEP 4: Simulator location detected - will show SF stores' : '🎯 STEP 4: Real location detected');

      // 3. Search for stores (even with simulator location to show it works)
